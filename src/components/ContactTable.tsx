...
interface ContactTableProps {
  ...
  onConsolidate: (contact: Contact) => void;
}

export function ContactTable({ contacts, onEdit, onDelete, onConsolidate }: ContactTableProps) {
  const [contactToDelete, setContactToDelete] = useState<string | null>(null);
  const [contactToConsolidate, setContactToConsolidate] = useState<Contact | null>(null);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'à relancer': return 'bg-orange-100 text-orange-800 border-orange-200';
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Action faite</th>
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {contacts.map((contact) => {
              const isConfirmed = contact.status === 'confirmé';
              return (
                <tr key={contact.id} className={isConfirmed ? 'bg-green-50/40' : 'hover:bg-slate-50 transition-colors'}>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                      ...
                      {!isConfirmed && (
                        <button onClick={() => setContactToConsolidate(contact)} className="flex items-center p-1.5 bg-green-50 text-green-600 hover:bg-green-100 rounded transition-colors" title="Consolider">
      ...
      {contactToConsolidate && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <div className="flex items-center mb-4 text-green-600">
              <CheckCircle className="w-6 h-6 mr-2" />
              <h3 className="text-lg font-semibold">Confirmer la consolidation</h3>
            </div>
            <p className="text-slate-600 mb-6">
              Voulez-vous vraiment consolider ce contact ? Il ne pourra plus être modifié ou supprimé.
            </p>
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => setContactToConsolidate(null)}
                className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg transition-colors font-medium"
              >
                ANNULER
              </button>
              <button
                onClick={() => {
                  onConsolidate(contactToConsolidate);
                  setContactToConsolidate(null);
                }}
